 ...
 */
public class RefacTextMergeViewer extends RefacContentMergeViewer implements IAdaptable {
	...
	class ContributorInfo implements IElementStateListener, VerifyListener, IDocumentListener, IEncodingSupport {
		private final RefacTextMergeViewer fViewer;
		...
		public ContributorInfo(RefacTextMergeViewer viewer, Object element, char leg) {
			fViewer = viewer;
			fElement = element;
			fLeg = leg;
				...
				if (parent instanceof IDocumentRange) {
					...
					if (range == null) {
						...
						try {
							...
							// silently ignored
							if (RefacTextMergeViewer.DEBUG) System.out.println("BadPositionCategoryException: " + ex);	//$NON-NLS-1$
						} catch (BadLocationException ex) {
							// silently ignored
							if (RefacTextMergeViewer.DEBUG) System.out.println("BadLocationException: " + ex);	//$NON-NLS-1$
						}
		...
		private void updateViewerDocumentRange(MergeSourceViewer tp, Position range) {
			...
			if (this.fViewer.fSubDoc) {
				if (range != null) {
					IRegion r= this.fViewer.normalizeDocumentRegion(tp.getSourceViewer().getDocument(), RefacTextMergeViewer.toRegion(range));
		...
		private void updateViewerDocument(MergeSourceViewer tp, IDocument document, Position range) {
			...
			if (this.fViewer.fSubDoc) {
				if (range != null) {
					IRegion r= this.fViewer.normalizeDocumentRegion(document, RefacTextMergeViewer.toRegion(range));
		...
		public void documentChanged(DocumentEvent e) {
			...
			RefacTextMergeViewer.this.documentChanged(e, dirty);
	 ...
	 */
	public RefacTextMergeViewer(Composite parent, CompareConfiguration configuration) {
